...
function dmg(scale, character, percentage){
    let Scale_total
    if (scale === "ATK"){
        Scale_total = character.ATK_base * character.ATK/100 + character.ATK_flat
    }
    if (scale === "DEF"){
       Scale_total = character.DEF_base * character.DEF/100 + character.DEF_flat
    }
    if (scale === "HP"){
        Scale_total = character.HP_base * character.HP/100 + character.HP_flat
     }
    return Scale_total * percentage/100 * character.elemental_bonus/100 * character.DEF_mob * character.RES_mob
}

function dmgCrit(DMG, character){
    return DMG * character.crit_damage/100
}
